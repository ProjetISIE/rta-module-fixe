...
Lidar::Lidar() {
    ...
    ESP_ERROR_CHECK(uart_driver_install(UartPort, RxBufferSize, 0, 0, nullptr, 0));
...
auto Lidar::read_descriptor(std::uint32_t timeout_ms) -> std::expected<void, Error> {
    std::uint8_t d[7];
    ...
    std::uint32_t raw = d[2] | (d[3] << 8) | (d[4] << 16) | (static_cast<std::uint32_t>(d[5]) << 24);
...
auto Lidar::read_point() -> std::expected<Point, Error> {
    ...
    std::uint16_t a_raw = ((d[1] >> 1) & 0x7F) | (static_cast<std::uint16_t>(d[2]) << 7);
