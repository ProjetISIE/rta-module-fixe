...
using rta::Degrees;
using rta::Millimeters;

static const char* TAG = "RTA_FIXE_MAIN";

// Angular Filter Configuration
static constexpr Degrees kAngleCenter{0.0f};
static constexpr Degrees kAngleHalfWidth{22.5f};
static constexpr Millimeters kDistMin{50};
static constexpr Millimeters kDistMax{14000};
static constexpr std::uint16_t kInvalidDistance = 0xFFFF;

[[nodiscard]] static constexpr auto normalize_angle(Degrees a) -> Degrees {
    while (a.value < 0.0f) a.value += 360.0f;
    while (a.value >= 360.0f) a.value -= 360.0f;
    return a;
}

[[nodiscard]] static constexpr auto is_in_sector(Degrees angle) -> bool {
    const float lo =
        normalize_angle(Degrees{kAngleCenter.value - kAngleHalfWidth.value})
            .value;
    const float hi =
        normalize_angle(Degrees{kAngleCenter.value + kAngleHalfWidth.value})
            .value;
...
void lidar_task(void*) {
    ...
    while (true) {
        ...
        if (auto res = sensor.start_scan(); !res) {
            ...
            rta::ble::invalidate_distance();
            rta::espnow::send_distance(kInvalidDistance);
        ...
        Millimeters min_dist{static_cast<std::uint16_t>(kDistMax.value + 1)};
        ...
        while (true) {
            ...
            if (point_res) {
                ...
                if (pt.start_flag) {
                    cnt_scans++;
                    if (min_dist <= kDistMax) {
                        rta::ble::update_distance(min_dist);
                        rta::espnow::send_distance(min_dist.value);
                    }
                    min_dist = Millimeters{
                        static_cast<std::uint16_t>(kDistMax.value + 1)};
                }

                if (is_in_sector(pt.angle)) {
                    if (pt.distance >= kDistMin && pt.distance <= kDistMax) {
            ...
            if ((xTaskGetTickCount() - last_valid_point_time) >
                pdMS_TO_TICKS(1000)) {
                if (consecutive_errors % 10 == 0) {
                    ESP_LOGW(TAG,
                             "LiDAR data timeout (1s), marking as invalid");
                }
                rta::ble::invalidate_distance();
                rta::espnow::send_distance(kInvalidDistance);
                last_valid_point_time =
            ...
            if (consecutive_errors >= MAX_CONSECUTIVE_ERRORS) {
                ESP_LOGE(
                    TAG,
                    "Too many consecutive LiDAR errors (%lu), restarting...",
                    consecutive_errors);
                break;
            }

            if ((xTaskGetTickCount() - t_stats) > pdMS_TO_TICKS(5000)) {
                ESP_LOGI(TAG, "LiDAR Stats: Scans: %lu, Total Pts: %lu",
                         cnt_scans, cnt_total);
...
extern "C" void app_main() {
    ...
    static rta::ble::BleServer ble_server("RTA_FIXE");

    if (ble_server.register_services(rta::ble::gatt_svcs) != 0) {
