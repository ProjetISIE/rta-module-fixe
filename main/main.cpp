...
[[nodiscard]] static constexpr auto is_in_sector(Degrees angle) -> bool {
    const float lo = normalize_angle(Degrees{ANGLE_CENTER.value - ANGLE_HALF_WIDTH.value}).value;
    const float hi = normalize_angle(Degrees{ANGLE_CENTER.value + ANGLE_HALF_WIDTH.value}).value;
        ...
        while (true) {
            ...
            if (point_res) {
                ...
                if (pt.start_flag) {
                    ...
                    min_dist = Millimeters{static_cast<std::uint16_t>(DIST_MAX.value + 1)};
                }
            ...
            if (consecutive_errors >= MAX_CONSECUTIVE_ERRORS) {
                ESP_LOGE(TAG, "Too many consecutive LiDAR errors (%lu), restarting...", consecutive_errors);
                break;
            }

            if ((xTaskGetTickCount() - t_stats) > pdMS_TO_TICKS(5000)) {
                ESP_LOGI(TAG, "LiDAR Stats: Scans: %lu, Total Pts: %lu", cnt_scans, cnt_total);
...
extern "C" void app_main() {
    ...
    static ble::Server ble_server("RTA_FIXE");
