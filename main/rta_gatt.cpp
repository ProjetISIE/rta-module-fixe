...
static constexpr ble_uuid128_t SERVICE_UUID = {
    ...
    .value = {0xF0, 0xDE, 0xBC, 0x9A, 0x78, 0x56, 0x34, 0x12, 0x34, 0x12, 0x78, 0x56, 0x34, 0x12, 0x78, 0x56}
};

static constexpr ble_uuid128_t CHAR_DIST_UUID = {
    .u = {.type = BLE_UUID_TYPE_128},
    .value = {0xF1, 0xDE, 0xBC, 0x9A, 0x78, 0x56, 0x34, 0x12, 0x34, 0x12, 0x78, 0x56, 0x34, 0x12, 0x78, 0x56}
};

static std::uint16_t char_dist_handle;
static Millimeters current_dist{0xFFFF}; // Sentinel value for NO DATA

extern "C" static int gatt_char_access(std::uint16_t conn_handle, std::uint16_t attr_handle,
...
extern "C" static int gatt_char_access(std::uint16_t conn_handle, std::uint16_t attr_handle,
    ...
    if (attr_handle == char_dist_handle) {
        return os_mbuf_append(ctxt->om, &current_dist.value, sizeof(current_dist.value));
    }
...
auto invalidate_distance() -> void {
    update_distance(Millimeters{0xFFFF});
}
