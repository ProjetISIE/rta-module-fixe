...
void lidar_init(void) {
    ...
    ESP_ERROR_CHECK(uart_set_pin(LIDAR_UART_PORT, LIDAR_TX_PIN, LIDAR_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
}
...
static bool lidar_read_descriptor(uint32_t *d_len, uint8_t *send_mode, uint8_t *d_type, uint32_t timeout_ms) {
    uint8_t d[7];
...
bool lidar_start_scan(void) {
    ...
    if (!lidar_read_descriptor(&d_len, &s_mode, &d_type, 2000)) return false;
    if (d_len != 5 || s_mode != 1) {
...
bool lidar_read_point(lidar_point_t *point) {
    ...
    if (S == Sb) return false; // Invalid start flags
